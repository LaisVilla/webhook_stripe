document.addEventListener('DOMContentLoaded', function() {
    // Seus elementos existentes do DOM
    ...
    // Variável para evitar múltiplos submits
        ...
        fetch('/api/user/ai-stats')
            ...
            .then(data => {
                if (data.status === 'success') {
                    // Atualizar requisições restantes
    ...
    aiForm.addEventListener('submit', async function(e) {
        ...
        if (isSubmitting) {
            console.log('Submissão duplicada bloqueada');
            return; // Impede múltiplas submissões simultâneas
        }
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(data.message || 'Erro ao processar sua requisição');
            }

            // Adicionar resposta do bot ao histórico
            addMessageToHistory(data.response);
    ...
    // Modificar a função copyToClipboard
    ...
    function addMessageToHistory(content, isUser = false, customTimestamp = null) {
        ...
        // Quebra linhas para melhor formatação
            ...
            <div class="flex items-start space-x-3 ${isUser ? 'justify-end' : ''}">
                ...
                <div class="flex-1">
                    <div class="bg-surface/30 rounded-xl p-4 ${
                        ...
                        <p class="text-gray-300 whitespace-pre-wrap">${formattedContent}</p>
                    </div>
